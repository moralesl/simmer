...
| `SIMMER_FAKE_CHECKOUT=<branch>:<default>:clean\|dirty` | what a working checkout looks like — the read that decides whether `--apply` may pull and rebuild the checkout a bundle was installed from. Anything that is not those three fields answers "cannot read this checkout", which refuses. Seamed for the reason `SIMMER_FAKE_LATEST` is: the real probe spawns `git` in a directory a fixture named, and a seamed process that has not been told what to answer reads nothing |
