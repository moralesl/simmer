...
- **A failure names the remote it fetched from, and never sends a person back to it.** Where the plan fetched a release from a remote (every `installer` plan), the `fetching` sentence names that remote and the `switching` sentence names the tag, the checkout and the remote it fetched from — where it looked, never why it failed, because the same step fails on a dirty tree and a sentence claiming the tag is absent would be a lie about that.
  `switching` is also the one phase that does not end in `Run: <update_command>`: for a bundle install that command is the one-paste installer, which fetches the same remote that just failed to yield the tag, so it names `git -C <checkout> status` instead.
