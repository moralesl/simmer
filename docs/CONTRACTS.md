...
**An `installer` plan fetches the tag from the repository the release was read from** — the same URL the one outbound request goes to — and not from that checkout's `origin`: `origin` is whatever cloned it, so on a Mac where it is a development checkout, or a mirror that lags, the fetch succeeded and the tag was not in what it fetched (`pathspec 'v0.3.3' did not match any file(s) known to git`).
A normal install's `origin` **is** that repository, so it does exactly what it did before; the remote is in the fetch step's own arguments, and therefore in `steps`, rather than in a field of its own.
A `checkout` plan keeps fetching its own `origin` — what it installs is what its default branch holds — and refuses rather than running whenever the tree is dirty, the head is detached, the branch is not the remote's default, or that default cannot be read locally — the read is local, so a checkout that cannot answer refuses instead of waiting on the network.
...
- **A failure names the remote it fetched from, and the `switching` sentence never sends a person back to it.** Where the plan fetched a release from a remote (every `installer` plan), the `fetching` sentence names that remote and the `switching` sentence names the tag, the checkout and the remote it fetched from — where it looked, never why it failed, because the same step fails on a dirty tree and a sentence claiming the tag is absent would be a lie about that.
  `switching` is the one phase that does not end in `Run: <update_command>`, and the promise is that narrow on purpose: for a bundle install that command is the one-paste installer, which fetches the same remote that just failed **to yield the tag**, so `switching` names `git -C <checkout> status` instead.
  `fetching` keeps the retry, because nothing was fetched — the remote was unreachable or answered nothing, and running the installer again is a reasonable thing to do about that; a fetch that failed and a fetch that succeeded without the release are different endings.
