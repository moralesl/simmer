...
- Piping a script from `curl` into `bash` means trusting this repository at the moment you run it.
  The script is short and does nothing before its final line; pin a release with `SIMMER_REF=v1.0.0` rather than tracking the default branch if that matters to you, and read it first — that is what the URL is for.
